...
export const ElementPanel: React.FC<ElementDrawerProps> = ({ id, element, metrics, queryOptions, onClose }) => {
  ...
  const metricTitle = React.useCallback(() => {
    if (queryOptions.metricFunction === 'rate') {
      return t('Flows rate');
    } else {
      switch (queryOptions.metricFunction) {
        case 'avg':
          return t('Average {{type}} (1m frame)', { type: queryOptions.metricType });
        case 'max':
          return t('Max {{type}} (1m frame)', { type: queryOptions.metricType });
        case 'sum':
          return t('Total {{type}}', { type: queryOptions.metricType });
        default:
          return '';
      }
    }
  }, [queryOptions.metricFunction, queryOptions.metricType, t]);

  const metricValue = React.useCallback(
    (v: number) => {
      return queryOptions.metricFunction !== 'rate' && queryOptions.metricType === 'bytes'
        ? humanFileSize(v, true, 0)
        : roundTwoDigits(v);
    },
    [queryOptions.metricFunction, queryOptions.metricType]
  );
