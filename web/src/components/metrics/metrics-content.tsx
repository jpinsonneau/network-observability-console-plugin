...
export const MetricsContent: React.FC<{
  ...
  const chart = React.useCallback(() => {
    ...
    function getNameValue(m: TopologyMetrics) {
      return `${getName(m)} ${((m.total / total) * 100).toFixed(2)}%`;
    }
    ...
    const containerComponent = (
      <ChartVoronoiContainer
        labels={({ datum }) =>
          datum.childName.startsWith('area-') || datum.childName.startsWith('bar-') ? `${datum.name}: ${datum.y}` : ''
        }
      ...
      >
        {showDonut ? (
          <ChartDonut
            ...
            height={350}
            data={metrics.map((m: TopologyMetrics) => ({ x: getNameValue(m), y: m.total }))}
            padding={{
              bottom: 20,
              left: 20,
              right: 300,
