...
export const getMetrics = (params: FlowQuery, range: number | TimeRange, step: number): Promise<MetricsResult> => {
  return axios.get(ContextSingleton.getHost() + '/api/loki/metrics', { params }).then(r => {
    ...
    const metrics = (aggQR.result as Metrics[]).flatMap(r =>
      calculateMatrixTotals(r, params.function as MetricFunction, range, step)
    );
    return {
