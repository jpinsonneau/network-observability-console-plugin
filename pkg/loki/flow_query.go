...
func NewFlowQueryBuilder(cfg *Config, start, end, limit string, reporter constants.Reporter, recordType constants.RecordType) *FlowQueryBuilder {
	...
	if recordType == constants.RecordTypeAllConnections {
		// connection _RecordType including newConnection, updateConnection or endConnection
		labelFilters = append(labelFilters, regexLabelFilter(constants.RecordTypeLabel, strings.Join([]string{
			string(constants.RecordTypeNewConnection),
			string(constants.RecordTypeUpdateConnection),
			string(constants.RecordTypeEndConnection),
		}, "|")))
	} else {
		// specific _RecordType either newConnection, updateConnection, endConnection or flowLog
