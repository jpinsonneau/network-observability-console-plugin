...
func TestQuery_ToURL_ConvertToAnyMatch(t *testing.T) {
	...
	for _, tc := range []testCase{{
		...
		expect:    `{app="flows",foo=~".*bar.*",baz=~".*bae.*"}`,
		expectAny: `{dont="fail"}|json|app="flows"+or+foo=~".*bar.*"+or+baz=~".*bae.*"`,
	}, {
		...
		expect:    "{app=\"netobs\"}|~`\"DstPort\":1234`|~`\"Namespace\":\".*hiya`",
		expectAny: "{dont=\"fail\"}|json|app=\"netobs\"+or+DstPort=1234+or+Namespace=~`.*hiya.*`",
	}, {
		...
		expect:    `{app="some-app"}|json|foo=~"bar"+or+port=1234+or+SrcAddr=ip("123.0.0.0/16")`,
		expectAny: `{dont="fail"}|json|app="some-app"+or+foo=~"bar"+or+port=1234+or+SrcAddr=ip("123.0.0.0/16")`,
	}, {
		...
		expect:    "{app=\"the-app\"}|~`\"DstPod\":\".*podaco\"`|json|foo=~\"bar\"",
		expectAny: "{dont=\"fail\"}|json|app=\"the-app\"+or+foo=~\"bar\"+or+DstPod=~`.*podaco.*`",
	}} {
...
func TestQuery_AddURLParam(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, `{app="the-app"}&foo=bar&flis=flas`, urlQuery)
}
